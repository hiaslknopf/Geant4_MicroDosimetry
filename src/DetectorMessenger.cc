	...
	: geometryHasChanged(false)
{
	...
	changeThePhantomCmd = new G4UIcmdWithAString("/geometrySetup/selectPhantom",this);
	changeThePhantomCmd -> SetGuidance("Select the type of phantom you wish to use");
	changeThePhantomCmd -> SetParameterName("Material",false);
	changeThePhantomCmd -> AvailableForStates(G4State_PreInit);
...
void DetectorMessenger::SetNewValue(G4UIcommand* command, G4String commandContent)
{
	...
	if ( command == changeThePhantomCmd )
	{
		if( commandContent == "Water" || commandContent == "RW3" || commandContent == "Vacuum")
		{
			phantomType = commandContent;
			geometryHasChanged = true;
			
			G4cout << "Phantom type changed to " << commandContent << G4endl;
			G4cout << "Run /geometrySetup/applyChanges to apply" << G4endl;
		}
		
		else
		{
			G4cout <<"Unknown phantom type: " << commandContent << ". Default to Vacuum phantom." << G4endl;
		}
	}
