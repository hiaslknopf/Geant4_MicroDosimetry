...
DetectorConstruction::DetectorConstruction(AnalysisManager* analysis_manager, DetectorMessenger* detector_messenger)
{
	...
	phantomType = messenger -> GetPhantomType();

	phantomThickness = messenger -> GetPhantomThickness();
	phantomWidth = messenger -> GetPhantomWidth();
...
{
	if (phantomType == "RW3") ConstructWorldWithRW3Phantom();
	else if (phantomType == "Water") ConstructWorldWithWaterPhantom();
	else if (phantomType == "Vacuum") ConstructVacuumWorld();
	else
	{
		G4cout << "ERROR: " << phantomType << " is not an allowed phantom type. ";
		G4cout << "Defaulting to vacuum world.";
		ConstructVacuumWorld();
	}

	if( detectorType == "CNMSiC" ) ConstructCNMSiCDetector();
	else if ( detectorType == "IISBSiC" ) ConstructIISBSiCDetector();
	else if( detectorType == "Diamond" ) ConstructDiamondDetector();
...
void DetectorConstruction::ConstructWorldWithRW3Phantom()
{
	//Define materials
	...
	G4double worldLength = phantomThickness + 5*cm;
	
	// In a clinical setup, the RW3 Phantom is surrounded by air
	...
	G4Box* phantom_box = new G4Box("phantom_box", phantomWidth/2, phantomWidth/2, phantomThickness/2);
	G4LogicalVolume* logical_phantom = new G4LogicalVolume(phantom_box, rw3, "phantom_log", 0,0,0);
	
	//Phantom starts at z=0
	G4ThreeVector phantom_position = G4ThreeVector( 0., 0., -phantomThickness/2 );
	...
	G4LogicalVolume* logical_inner = new G4LogicalVolume(inner_box, air, "inner_log",0,0,0);
	
	G4double innerDepth = phantomThickness/2 + 2.5*cm; //2.5cm behind the RW3
...
void DetectorConstruction::ConstructCNMSiCDetector()
{}

void DetectorConstruction::ConstructIISBSiCDetector()
{}
